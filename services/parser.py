...
=== 섹션 구분 원칙 (중요) ===
=== 섹션 구분 원칙 (중요) ===
1. type=intro: 교재의 시작 부분(III. 말씀 정리하기)의 서술형 문단만 포함합니다.
   - text: 텍스트 박스 안의 도입 단락 전체.
   - items: (권장) 비워두거나, 본격적인 번호 질문(1.)이 나오기 전의 배경 설명만 넣습니다. **번호가 붙은 질문은 절대 여기에 넣지 마세요.**

2. type=point: "1. ", "2. ", "3. " 등 대문항 번호로 시작하는 모든 학습 질문 블록입니다.
   - **교재의 첫 번째 1번 질문부터 반드시 point 섹션으로 생성해야 합니다.**
...
- level: 정수 (1: 중문항 (1), (2) / 2: 소문항 1), 2) / 3: 세부항목).
- text: 정제된 텍스트. **(중요: 문장 중간의 빈칸 ( )이나 정답으로 인해 문장을 여러 개로 쪼개지 마세요. 하나의 완성된 문장으로 추출해야 합니다.)**

=== 텍스트 정제 규칙 ===
- 문장 시작의 기호("1.", "(1)", "1)", "-", "•", "▶")만 제거합니다.
- 문장 중간에 있는 "( )" 또는 정답 단어는 텍스트의 일부로 그대로 유지합니다.
